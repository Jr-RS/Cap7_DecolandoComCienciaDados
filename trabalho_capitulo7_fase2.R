...
cat("Média:", media_volume, "\n") 
cat("Mediana:", mediana_volume, "\n")
cat("Moda:", moda_volume, "\n")
...
cat("Máximo:", maximo_volume, "\n")
cat("Mínimo:", minimo_volume, "\n")
cat("Variância:", variancia_volume, "\n")
...
ggplot(data = dados_volume, aes(x = codigos, y = volume_colhido)) + geom_bar(stat = "identity", fill = "pink") + labs(title = "Volume de Milho Colhido", x = "Códigos das Sementes",y = "Volume (Toneladas)")
...
#Criando um data frame
dados_qualid <- data.frame(Sementes_Avaliadas = 1:30, Qualidade_Grãos = qualid_graos)

ggplot(data = dados_qualid, aes(x = Sementes_Avaliadas, y = qualid_graos)) + geom_bar(stat = "identity", fill = "blue") + labs(title = "Qualidade dos Grãos Colhidos", x = "Quantidade de Sementes Avaliadas",y = "Qualidade dos Grãos")
